...
| ---------------- | -------- | ------------------------------- | -------------------------------------------------------------------- |
| Content-Type     | 可选     | application/json; charset=utf-8 | POST方法提交需要设置为application/x-www-form-urlencoded;charset=utf-8<br />使用 PostMan 调用 upload/post 接口提交附件时需要设置为 form-data<br />服务端始终返回 JSON 格式内容。 |
